...
class Bill_App:
    def __init__(self, root):
        ...
        self.face_cleanser = IntVar()
        self.face_wash = IntVar()
        self.hair_colour = IntVar()
        self.body_lotion = IntVar()
        self.rice = IntVar()
        self.daal = IntVar()
        self.olive_oil = IntVar()
        self.atta = IntVar()
        self.sugar = IntVar()
        self.maza = IntVar()
        self.coke = IntVar()
        ...
        # =======Face Cream
        face_lbl = Label(F2, font=("times new roman", 15, "bold"), fg=lbl_color, bg=bg_color, text="Face Cleanser")
        face_lbl.grid(row=1, column=0, padx=10, pady=20)
        face_en = Entry(F2, bd=8, relief=GROOVE, textvariable=self.face_cleanser)
        ...
        # ========Hair Spray
        hair_lbl = Label(F2, font=("times new roman", 15, "bold"), fg=lbl_color, bg=bg_color, text="Hair Colour")
        hair_lbl.grid(row=3, column=0, padx=10, pady=20)
        hair_en = Entry(F2, bd=8, relief=GROOVE, textvariable=self.hair_colour)
        ...
        # =======
        oil_lbl = Label(F2, font=("times new roman", 15, "bold"), fg=lbl_color, bg=bg_color, text="Olive Oil")
        oil_lbl.grid(row=1, column=0, padx=10, pady=20)
        oil_en = Entry(F2, bd=8, relief=GROOVE, textvariable=self.olive_oil)
        ...
        # ========
        wheat_lbl = Label(F2, font=("times new roman", 15, "bold"), fg=lbl_color, bg=bg_color, text="Atta")
        wheat_lbl.grid(row=3, column=0, padx=10, pady=20)
        wheat_en = Entry(F2, bd=8, relief=GROOVE, textvariable=self.atta)
        ...
        # ========
        cold_lbl = Label(F2, font=("times new roman", 15, "bold"), fg=lbl_color, bg=bg_color, text="Chips")
        cold_lbl.grid(row=3, column=0, padx=10, pady=20)
        cold_en = Entry(F2, bd=8, relief=GROOVE, textvariable=self.chips)
    ...
    def total(self):
        ...
        self.total_cosmetics_prices = (
                ...
                (self.face_cleanser.get() * 140) +
                (self.face_wash.get() * 240) +
                (self.hair_colour.get() * 340) +
    ...
    def bill_area(self):
        ...
        if self.face_cleanser.get() != 0:
            self.txt.insert(END, f"\nFace Cream        {self.face_cleanser.get()}           {self.face_cleanser.get() * 140}")
        if self.face_wash.get() != 0:
            self.txt.insert(END, f"\nFace Wash         {self.face_wash.get()}           {self.face_wash.get() * 240}")
        if self.hair_colour.get() != 0:
            self.txt.insert(END, f"\nHair Spray        {self.hair_colour.get()}           {self.hair_colour.get() * 340}")
        if self.body_lotion.get() != 0:
            self.txt.insert(END,
                            f"\nBody Lotion       {self.body_lotion.get()}           {self.body_lotion.get() * 260}")
        if self.atta.get() != 0:
            self.txt.insert(END, f"\nWheat             {self.atta.get()}           {self.atta.get() * 100}")
        if self.olive_oil.get() != 0:
            self.txt.insert(END, f"\nFood Oil          {self.olive_oil.get()}           {self.olive_oil.get() * 180}")
        if self.daal.get() != 0:
        ...
        if self.chips.get() != 0:
            self.txt.insert(END, f"\nNimko             {self.chips.get()}           {self.chips.get() * 20}")
